...
import ru.congas.core.input.keys.AnyKey;
...
public abstract class AbstractWindowsTerminal extends AbstractTerminal {
    ...
    private final InputThread inputHandler;

    public AbstractWindowsTerminal(Writer writer, String name, String type, Charset encoding, int codepage, boolean nativeSignals, SignalHandler signalHandler, InputThread inputHandler) throws IOException {
    ...
    protected void processKeyEvent(final boolean isKeyDown, final short virtualKeyCode, char ch, final int controlKeyState) throws IOException {
        ...
        if (isKeyDown && ch != '\3') {
            ...
            if (ch != 0 && (controlKeyState & (RIGHT_ALT_PRESSED | LEFT_ALT_PRESSED | RIGHT_CTRL_PRESSED | LEFT_CTRL_PRESSED | SHIFT_PRESSED))
                        == (RIGHT_ALT_PRESSED | LEFT_CTRL_PRESSED)) {
                //inputHandler.safeHandle(ch, isAlt, isShift, isCtrl);
                processInputChar(ch);
            } else {
                AnyKey k = Key.getKey((char) virtualKeyCode);
                //if (key == 13) key = 0;
                inputHandler.safeHandle(new KeyPressed(k, ch == 0 ? null : ch, isShift, isCtrl, isAlt));
    ...
    public void resume() {
        synchronized (lock) {
            paused = false;
            if (pump == null) {
                pump = new Thread(this::pump, "WinInput");
                pump.setDaemon(true);
                pump.start();
            }
        }
